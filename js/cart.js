...
export function addToCart(item) {
  if (!item || !item.sku) {
    ...
    showToast('⚠️ Error al agregar al carrito', 'error');
    return;
  }
  ...
  if (existingIndex !== -1) {
    ...
    cart.push({
      ...
      imagen: item.imagen || 'Garofano_Blu.png'
    });
  }
...
export function updateCartItemQuantity(sku, quantity) {
  ...
  quantity = Math.max(0, Math.min(parseInt(quantity) || 0, CONFIG.MAX_QUANTITY_PER_SKU));
...
function loadCart() {
  try {
    ...
    if (saved) {
      ...
      cart = cart.filter(item => item && item.sku && item.cantidad > 0 && item.precio > 0);
    }
  } catch (error) {
...
export function renderCart() {
  ...
  const validCart = cart.filter(item => item && item.sku && item.cantidad > 0 && item.precio > 0);
  
  if (validCart.length === 0) {
    cartItemsContainer.innerHTML = `
      <div class="empty-cart">
        <p>No hay productos agregados todavía</p>
        <button class="btn-secondary close-cart" type="button">Sigue comprando</button>
      </div>
    `;
...
function createCartItem(item) {
  ...
  cartItem.innerHTML = `
    <div class="cart-item-image">
      <img src="${imageUrl}" alt="${item.coleccion || 'Producto'}" loading="lazy">
    </div>
    ...
    <div class="cart-item-quantity">
      <button class="qty-btn qty-minus" data-sku="${item.sku}" aria-label="Disminuir cantidad">-</button>
      <input type="number" class="qty-input" id="cart-qty-${item.sku}" name="qty-${item.sku}" value="${item.cantidad || 0}" min="0" max="${CONFIG.MAX_QUANTITY_PER_SKU}" data-sku="${item.sku}" aria-label="Cantidad">
      <button class="qty-btn qty-plus" data-sku="${item.sku}" aria-label="Aumentar cantidad">+</button>
    </div>
